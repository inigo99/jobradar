...
function mailBlock(job) {
  ...
  const box = el("div", { className: "mailnews " + news.kind },
    el("div", {},
      el("span", { className: "chip mail-" + news.kind }, MAIL_KIND_LABEL[news.kind] || news.kind),
      " ", el("span", { className: "hint" }, `${when} · ${news.sender}`)),
    el("div", {}, el("b", {}, news.subject)),
    news.excerpt ? el("blockquote", {}, news.excerpt) : null,
    news.link ? el("a", { href: news.link, target: "_blank", rel: "noopener" }, "Open the thread") : null,
  );
  if (news.kind === "rejection" && job.stage !== "rejected") {
    box.append(el("p", { className: "hint" },
      "This reads like a rejection. If it is, mark it so it stops counting as a live process.",
      " ", button("Mark as rejected", () =>
  ...
  if (news.interview_at) {
    ...
    box.append(el("p", {},
      el("b", {}, "Proposed interview: "),
      at.toLocaleString(undefined, { dateStyle: "full", timeStyle: "short" }), " ",
...
function orphansBlock() {
  ...
  return el("div", { className: "panel orphans" },
    el("h3", {}, `${orphans.length} ${orphans.length === 1 ? "reply" : "replies"} in your email ` +
      "about applications not marked as applied here"),
    el("p", { className: "hint" },
      "If one matches a job on the board, open it and mark it as applied so its replies are " +
      "tracked; otherwise it was probably an application made elsewhere."),
    el("ul", {}, ...orphans.slice(0, 15).map(news =>
      el("li", {},
        ...
        news.link ? [" ", el("a", { href: news.link, target: "_blank", rel: "noopener" }, "open")] : null))),
  );
}

async function checkMail() {
  const node = $("#btn-mail");
  node.disabled = true; node.textContent = "Reading mail…";
  try {
    const result = await api("/api/mail/check", { method: "POST" });
    await refresh();
    toast(result.summary, 6000);
  } catch (error) { toast(error.message, 8000); }
  node.disabled = false; node.textContent = "Check email";
}
